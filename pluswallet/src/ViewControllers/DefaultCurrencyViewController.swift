...
class DefaultCurrencyViewController: UITableViewController, Subscriber {
    ...
    init(walletManager: BTCWalletManager) {
        ...
        self.rates = Currencies.btc.state?.rates.filter { $0.code != Currencies.btc.code && $0.code != Currencies.bch.code } ?? [Rate]()
    ...
    private let rateLabel = UILabel(font: .customBody(size: 18.0), color: .white)
    private var header: UIView?
    private let separatorView = UIView(color: .black)
    deinit {
...
//        navigationItem.rightBarButtonItems = [UIBarButtonItem.negativePadding, UIBarButtonItem(customView: faqButton)]
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let rateLabelTitle = UILabel(font: .customBold(size: 16.0), color: .white)
...
//            rateLabelTitle.leadingAnchor.constraint(equalTo: header.leadingAnchor, constant: C.padding[2]),
        ...
        rateLabel.constrain([
           // rateLabel.leadingAnchor.constraint(equalTo: rateLabelTitle.leadingAnchor),
            ...
            rateLabel.topAnchor.constraint(equalTo: rateLabelTitle.bottomAnchor, constant: C.padding[1]) ,
            rateLabel.bottomAnchor.constraint(equalTo: header.bottomAnchor, constant: -C.padding[2])])
        separatorView.constrain([
            separatorView.leadingAnchor.constraint(equalTo: header.leadingAnchor),
            separatorView.trailingAnchor.constraint(equalTo: header.trailingAnchor),
            separatorView.bottomAnchor.constraint(equalTo: header.bottomAnchor),
