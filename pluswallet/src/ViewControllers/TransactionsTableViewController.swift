...
class TransactionsTableViewController: UITableViewController, Subscriber, Trackable {
    ...
    // khi chua co transaction se xuat hien dong chu "transaction cua ban se dc hien thi o day"
    private let emptyMessage = UILabel.wrapping(font: .customBody(size: 16.0), color: .grayTextTint)

    //TODO:BCH replace with recommend rescan / tx failed prompt
    private var currentPrompt: Prompt? {// dang cho chuyen tien
        didSet {
           // print("day la currentPromt 99999999999999999999999999999999999999999 \(currentPrompt)")
...
//        tableView.estimatedRowHeight = 64.0 // 100 cung k anh huong
//        tableView.rowHeight = UITableViewAutomaticDimension
    ...
    private func setupSubscriptions() {
        ...
        Store.subscribeAsTrigger(self, name: .txMemoUpdated(""), callback: {
    ...
    private func setContentInset() {
        let insets = UIEdgeInsets(top: 0, left: 0, bottom: accountFooterHeight + C.padding[2], right: 0) // vi co footer nen xoa di 64
...
// MARK: - Cell Builders , cau hinh cho tung Header cell , TransactionCell
extension TransactionsTableViewController {
    ...
    private func transactionCell(tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: transactionCellIdentifier, for: indexPath) as? TxListCell else {
            return TxListCell()
        }
