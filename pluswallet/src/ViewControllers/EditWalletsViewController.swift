...
class EditWalletsViewController: UIViewController {
    ...
    private func addMenuButton() {
        ...
        menuButton.constrain(toSuperviewEdges: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0))
        menuButton.tap = {
            self.pushAddWallets()
        }
    }
    ...
    private func setAddModel(storedCurrencies: [CurrencyDef]) {
        model = storedCurrencies.filter {
            guard let token = $0 as? ERC20Token else {
                return false
            }
            return self.metaData.previouslyAddedTokenAddresses.contains(token.address)
        }.map {($0, true)}
        allModels = model
    }
    ...
    private func addAddedTokens() {
        let tokensToBeAdded: [ERC20Token] = model.filter { $0.1 == false }.map {
            guard let token = $0.0 as? ERC20Token else {
                fatalError("Added Token is not ERC20Token")
            }
            return token
        }
