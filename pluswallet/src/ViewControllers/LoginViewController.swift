...
class LoginViewController: UIViewController, Subscriber, Trackable {
    ...
    private let backgroundView = UIImageView(image: UIImage(named: "plus_wallet_background"))
    private let pinPad = PinPadViewController(style: PinPadStyle.clear, keyboardType: .pinPad, maxDigits: 0)
...
//        }
        ...
        Store.subscribeAsTrigger(self, name: .loginFromSend, callback: {_ in
            self.authenticationSucceded()
        })
    }
