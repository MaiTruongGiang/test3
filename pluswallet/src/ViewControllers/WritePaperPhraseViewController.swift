...
class WritePaperPhraseViewController: UIViewController {
    ...
    private lazy var phraseViews: [PhraseView] = {// lay 12 chu nhap vao mang UIview
    ...
    private let proceed = ShadowButton(title: S.WritePaperPhrase.next, type: .secondary) // nut toi
    private let previous = ShadowButton(title: S.WritePaperPhrase.previous, type: .secondary)// nut lui
    ...
    override func viewDidLoad() {
        ...
        label.text = S.WritePaperPhrase.instruction // value:"Write down each word in order and store it in a safe place."
...
//        navigationController?.navigationBar.isTranslucent = false
    ...
    private func addSubviews() {
        view.addSubview(header)// khung vien mau xanh
    ...
    private func addConstraints() {
        ...
        if #available(iOS 11.0, *) {
            header.constrain([
                ...
                header.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 0.0),
                header.trailingAnchor.constraint(equalTo: view.trailingAnchor),
                header.heightAnchor.constraint(equalToConstant: height) ])
        } else {
            header.constrain([
                header.leadingAnchor.constraint(equalTo: view.leadingAnchor),
                header.topAnchor.constraint(equalTo: topLayoutGuide.bottomAnchor, constant: 0.0),
                header.trailingAnchor.constraint(equalTo: view.trailingAnchor),
                header.heightAnchor.constraint(equalToConstant: height) ])
        }

//        header.constrainTopCorners(sidePadding: 0, topPadding: 0) // can chinh top, voi trai phai
//        header.constrain([
//        header.constraint(.height, constant: 100.0) ])
        label.constrainBottomCorners(sidePadding: C.padding[3], bottomPadding: C.padding[2])
        label.constrain([
            label.leadingAnchor.constraint(equalTo: header.leadingAnchor, constant: C.padding[3]),
            label.trailingAnchor.constraint(equalTo: header.trailingAnchor, constant: -C.padding[3]),
