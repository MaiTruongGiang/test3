...
class SupportCenterContainer: UIViewController {
    ...
    init(walletManagers: [String: WalletManager]) {
        ...
        #if Debug || Testflight
            webView = BRWebViewController(bundleName: "brd-web-staging", mountPoint: mountPoint, walletManagers: walletManagers)
        #else
            webView = BRWebViewController(bundleName: "brd-web", mountPoint: mountPoint, walletManagers: walletManagers)
        #endif
        webView.startServer()
        webView.preload()
