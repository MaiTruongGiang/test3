...
class EthWalletManager: WalletManager {
    ...
    init?() {
        ...
        Store.perform(action: WalletChange(self.currency).set(self.currency.state!.mutateAddress(receiveAddress: address)))
...
extension EthWalletManager: EthereumClient {
    ...
    func getBalance(wallet: EthereumWallet, address: String, completion: @escaping (String) -> Void) {
        ...
        if let token = currency as? ERC20Token {
            ...
            guard syncState.willBeginRequest(.getBalance, currencies: [currency]) else {
                return print("getBalance \(currency.code) skipped")
            }
