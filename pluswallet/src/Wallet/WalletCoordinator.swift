...
class WalletCoordinator: Subscriber, Trackable {
    ...
    private func addSubscriptions() {
        ...
        Store.state.currencies.forEach { currency in
            Store.subscribeAsTrigger(self, name: .retrySync(currency), callback: { [weak self] _ in
                DispatchQueue.walletQueue.async {
                    self?.walletManagers[currency.code]?.peerManager?.connect()
                }
            })

            Store.subscribeAsTrigger(self, name: .rescan(currency), callback: { [weak self] _ in
