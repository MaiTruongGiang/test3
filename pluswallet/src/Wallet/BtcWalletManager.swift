    ...
    func initWallet(callback: @escaping (Bool) -> Void) {
        db?.loadTransactions { txns in
            ...
            if let wallet = self.wallet {
                ...
                Store.perform(action: WalletChange(self.currency).set(self.currency.state!.mutateAddress(receiveAddress: wallet.receiveAddress)))
            }
        ...
        if let oldBalance = currency.state?.balance?.asUInt64 {
            if newBalance > oldBalance {
                if let walletState = currency.state {
                    Store.perform(action: WalletChange(currency).set(walletState.mutateAddress(receiveAddress: wallet?.receiveAddress)))
