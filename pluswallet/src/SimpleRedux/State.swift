...
//                                Currencies.brd.code: WalletState.initial(Currencies.brd, displayOrder: -1),
    ...
    func mutate(   isStartFlowVisible: Bool? = nil,
        ...
        return State(isStartFlowVisible: isStartFlowVisible ?? self.isStartFlowVisible,
                     ...
                     userState: userState ?? self.userState
        )
    }
    ...
    func mutateUser(userState: UserState) -> State {
        return mutate(userState: userState)
    }
}
...
struct WalletState {
    ...
    func mutateAddress(receiveAddress: String?) -> WalletState {
        return mutate(receiveAddress: receiveAddress)
    }
}
...
////////////
struct UserState {
    let imageFromGallery: Bool
    let imageURL: String
    var avatar: Data
    let userID: String
    let userName: String

    static func initial() -> UserState {
        return UserState(imageFromGallery: UserDefaults.profileFromGallery,
                         imageURL: UserDefaults.profileURL,
                         avatar: UserDefaults.profileAvatar as Data,
                         userID: UserDefaults.profileId,
                         userName: UserDefaults.profileName
        )

}
    func mutate(imageFromGallery: Bool? = nil,
                imageURL: String? = nil,
                avatar: Data? = nil,
                userID: String? = nil,
                userName: String? = nil) -> UserState {

        return UserState(imageFromGallery: imageFromGallery ?? self.imageFromGallery,
...
func ==(lhs: UserState, rhs: UserState) -> Bool {
    return lhs.imageFromGallery == rhs.imageFromGallery &&
