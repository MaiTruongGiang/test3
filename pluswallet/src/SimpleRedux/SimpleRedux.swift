...
enum TriggerName {
    ...
    case loadContactInfo(String?, String?)
    case uploadAvatar(String?, Data?)
    case didWalletAddressChanged(CurrencyDef)
...
func ==(lhs: TriggerName, rhs: TriggerName) -> Bool {
    ...
    case (.uploadAvatar, .uploadAvatar):
        return true
    case (.loadContactInfo, .loadContactInfo):
        return true
    case (.didWalletAddressChanged, .didWalletAddressChanged):
        return true
    default:
        return false
    }
...
class Store {
    ...
    static func subscribeAsTrigger(_ subscriber: Subscriber, name: TriggerName, callback: @escaping (TriggerName?) -> Void) {
        Store.shared.subscribeAsTrigger(subscriber, name: name, callback: callback)
    }
    ...
    func perform(action: Action) {
        state = action.reduce(Store.state)
    }
    ...
    func subscribeAsTrigger(_ subscriber: Subscriber, name: TriggerName, callback: @escaping (TriggerName?) -> Void) {
        let key = subscriber.hashValue
