...
enum ProfileChange {
    ...
    struct setProfileId: Action {
        let reduce: Reducer
        init(_ id: String) {
            UserDefaults.profileId = id
            reduce = { $0.mutateUser(userState: $0.userState.mutate(userID: id)) }
        }
    }

    struct setProfileUrl: Action {
        let reduce: Reducer
        init(_ url: String, isFromGallery: Bool) {
            UserDefaults.profileURL = url
            UserDefaults.profileFromGallery = isFromGallery
            reduce = { $0.mutateUser(userState: $0.userState.mutate(imageFromGallery: isFromGallery, imageURL: url)) }
        }
    }
    ...
    struct setUsername: Action {
        ...
        init(_ username: String) {
            UserDefaults.profileName = username
            reduce = { $0.mutateUser(userState: $0.userState.mutate(userName: username)) }
        }
    }

    struct setProfileAvatar: Action {
        let reduce: Reducer
        init(_ data: Data) {
            UserDefaults.profileAvatar = data
            reduce = { $0.mutateUser(userState: $0.userState.mutate(avatar: data)) }
        }
    }
}
