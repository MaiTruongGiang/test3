...
extension BRAPIClient {
    ...
    public func getEthTxList(address: EthAddress, handler: @escaping (APIResult<[EthTxJSON]>) -> Void) {
        let req = URLRequest(url: URL(string: "\(etherscanAPI)?module=account&action=txlist&address=\(address)&startblock=0&endblock=99999999&sort=asc")!)
    ...
    public func getTokenBalance(address: EthAddress, token: ERC20Token, handler: @escaping (APIResult<String>) -> Void) {
        let req = URLRequest(url: URL(string: "\(host):\(port)/getBalanceToken?address=\(address)&contractaddress=\(token.address)")!)
        send(apiRequest: req, handler: handler)
    }
    ...
    public func getTokenTransferLogs(address: EthAddress, contractAddress: String?, handler: @escaping (APIResult<[EthLogEventJSON]>) -> Void) {
        ...
        let req = URLRequest(url: URL(string: "http://13.231.62.98:3000/logs?module=logs&action=getLogs&fromBlock=0&toBlock=latest\(tokenAddressParam)&topic0=\(ERC20Token.transferEventSignature)&topic1=\(accountAddress)&topic1_2_opr=or&topic2=\(accountAddress)")!)
    ...
    private func send<ResultType>(rpcRequest: JSONRPCRequest, handler: @escaping (JSONRPCResult<ResultType>) -> Void) {
        ...
        var req = URLRequest(url: URL(string: "\(host):8545")!)
        req.httpMethod = "POST"
    ...
    private func send<ResultType>(apiRequest: URLRequest, handler: @escaping (APIResult<ResultType>) -> Void) {
        dataTaskWithRequest(apiRequest, authenticated: true, retryCount: 0, handler: { data, _, error in
