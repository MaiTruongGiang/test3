...
extension BRAPIClient {
    ...
    func me() {
        ...
        let task = dataTaskWithRequest(req, authenticated: true, handler: { data, _, _ in
            if let data = data {
...
//        let req = URLRequest(url: url("/fee-per-kb\(param)"))
        ...
        let req = URLRequest(url: URL(string: "https://stage2.breadwallet.com/fee-per-kb\(param)")!)
    ...
    func bitcoinExchangeRates(isFallback: Bool = false, _ handler: @escaping (RatesResult) -> Void) {
        ...
        let strURL = "\(bitpayAPI)/rates/\(code)"
        let task = dataTaskWithRequest(URLRequest(url: URL(string: strURL)!)) { (data, _, error) in
    ...
    func tokenExchangeRates(_ handler: @escaping (RatesResult) -> Void) {
        let request = URLRequest(url: URL(string: "\(coinmarketAPI)/ticker/?convert=BTC")!)
