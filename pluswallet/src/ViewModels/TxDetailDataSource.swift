...
extension TxDetailDataSource: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch field {
        case .amount:
            guard let amountCell = cell as? TxAmountCell else {
                return TxAmountCell()
            }
            amountCell.set(viewModel: viewModel)

        case .status:
            guard let statusCell = cell as? TxStatusCell else {
                return TxStatusCell()
            }
            statusCell.set(txInfo: viewModel)

        case .memo:
            guard let memoCell = cell as? TxMemoCell else {
                return TxMemoCell()
            }
            memoCell.set(viewModel: viewModel, tableView: tableView)

        case .timestamp:
            guard let labelCell = cell as? TxLabelCell else {
                return TxLabelCell()
            }
        ...
        case .address:
            guard let addressCell = cell as? TxAddressCell else {
                return TxAddressCell()
            }
