...
class AccountHeaderView: UIView, GradientDrawable, Subscriber {
    ...
    init(currency: CurrencyDef) {
        ...
        self.symbol.image = UIImage(named: currency.code.lowercased())
...
//      addSubview(modeLabel)
    ...
    private func addConstraints() {
        symbol.constrain([
                        symbol.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -C.padding[3]),
                        symbol.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: C.padding[3]),
