...
//        subTitlelbl.constrain([
//            subTitlelbl.constraint(.centerX, toView: self, constant: 0.0),
//            subTitlelbl.constraint(.bottom, toView: self, constant: -C.padding[2]) ])
        ...
        if style == .transaction {
            ...
            close.constrain([
                close.constraint(.trailing, toView: self, constant: 0.0),
                close.constraint(.centerY, toView: self, constant: E.isIPhoneX ? -10.0 : -20.0 ),
                close.constraint(.height, constant: buttonSize),
                close.constraint(.width, constant: buttonSize) ])
        }
//        guard let viewColor  = currency?.colors.0 else {    = .white}
        backgroundColor =  (currency?.colors.0) ?? .white
