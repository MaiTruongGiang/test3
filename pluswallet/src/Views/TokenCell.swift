...
//    private let header = UILabel(font: .customBold(size: 18.0), color: UIColor.fromHex("546875"))
    private let header = UILabel(font: .customBold(size: 18.0), color: UIColor.fromHex("548625"))
    private let subheader = UILabel(font: .customBody(size: 16.0), color: UIColor.fromHex("456875"))
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        setupViews()
        switchbtn.addTarget(self, action: #selector(switchStateDidChange(_:)), for: UIControlEvents.valueChanged)
    }
...
//            button.widthAnchor.constraint(equalToConstant: 80.0)])
        switchbtn.constrain([
            switchbtn.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -C.padding[2]),
            switchbtn.centerYAnchor.constraint(equalTo: contentView.centerYAnchor),
            switchbtn.heightAnchor.constraint(equalToConstant: 40.0),
            switchbtn.widthAnchor.constraint(equalToConstant: 80.0)])
    }
